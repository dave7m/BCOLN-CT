  ...
  return (
      <div className="py-16 px-5 bg-slate-100 space-y-10">
        ...
        <div className="flex flex-col items-center justify-center text-center space-y-3">
          ...
          <p className="text-lg text-slate-600 font-semibold capitalize">{jackpot?.title}</p>
        ...
        <div className="bg-white rounded-xl shadow-md w-full sm:w-3/4 mx-auto p-6">
          <h4 className="text-2xl font-bold text-slate-700 text-center mb-4">Winners & Losers</h4>
          <div className="space-y-2 max-h-80 overflow-y-auto">
            {participants?.map((participant, i) => (
                ...
                >
                  <Identicon size={30} string={i} className="rounded-full h-8 w-8" />
                  <div className="flex flex-wrap items-center gap-4 text-sm text-slate-600">
                    ...
                    {result?.winners.includes(participant.lotteryNumber) ? (
                        <p className="text-green-500 flex items-center font-semibold">
                          + <FaEthereum className="mx-1" /> {result?.sharePerWinner} <span className="ml-1">Winner</span>
                        </p>
                    ) : (
